                        ...
                        <tbody>
                            @foreach ($pemesanans as $p)
                                <tr>
                                    ...
                                    <td>
                                        @if ($p->detail_pemesanan && $p->detail_pemesanan->count() > 0)
                                    ...
                                    <td>
                                        Rp
                                        @if ($p->detail_pemesanan && $p->detail_pemesanan->count() > 0)
                                            {{ number_format($p->detail_pemesanan->sum('harga_subtotal'), 0, ',', '.') }}
                                        @else
                                    ...
                                    <td>
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('admin.transaksi.detail_pemesanan.index') }}" class="btn btn-info btn-sm" title="Lihat Detail">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('admin.transaksi.pemesanan.edit', $p->id) }}" class="btn btn-warning btn-sm" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('admin.transaksi.pemesanan.destroy', $p->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pemesanan ini?')" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
